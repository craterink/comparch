...
int setNthBit(int number, int n, int bitVal) {
	if(bitVal) {
		return (number | (1 << n));
	}
	else {
...
void setCC_N(int val) {
	val = Lowbit(val);
	NEXT_LATCHES.PSR = setNthBit(NEXT_LATCHES.PSR, PSR_NBIT, val);
	NEXT_LATCHES.N = val;
}
void setCC_Z(int val) {
	val = Lowbit(val);
	NEXT_LATCHES.PSR = setNthBit(NEXT_LATCHES.PSR, PSR_ZBIT, val);
	NEXT_LATCHES.Z = val;
}
void setCC_P(int val) {
	val = Lowbit(val);
	NEXT_LATCHES.PSR = setNthBit(NEXT_LATCHES.PSR, PSR_PBIT, val);
	NEXT_LATCHES.P = val;
}

void loadPSR_CC(int psr) {
	NEXT_LATCHES.N = NthBit(PSR_NBIT, psr);
	NEXT_LATCHES.Z = NthBit(PSR_ZBIT, psr);
	NEXT_LATCHES.P = NthBit(PSR_PBIT, psr);	
}
...
void storeReg(int regNum, int value){
	...
	else if (regNum == N_CC)                  setCC_N(value);
	else if (regNum == Z_CC)                  setCC_Z(value);
	else if (regNum == P_CC)                  setCC_P(value);
	else                                      exit(SIM_ERROR);
}
...
int GetLD_SSTK(int *cuinstr) {
	return GetLD_FORCESSTK(cuinstr) || 
		(GetLD_STK(cuinstr) && !(NthBit(15, NEXT_LATCHES.PSR)));
}
int GetLD_USTK(int *cuinstr) {
	return GetLD_STK(cuinstr) && NthBit(15, NEXT_LATCHES.PSR);
}
...
void latch_datapath_values() {
	...
	PUSHMUX_OUT = GetPUSHMUX(cuinstr) ? (GATE_STK_IN - 2) : (GATE_STK_IN + 2);
	STKINMUX_OUT = GetSTKINMUX(cuinstr) ? PUSHMUX_OUT : SR1_OUT;
	if(GetLD_USTK(cuinstr)) {
		NEXT_LATCHES.USP = Low16bits(STKINMUX_OUT);
	}
	if(GetLD_SSTK(cuinstr)) {
		NEXT_LATCHES.SSP = Low16bits(STKINMUX_OUT);
	}
	if(GetLD_INTV(cuinstr)) {
		VECMUX_OUT = VECLOOKUP[GetVECMUX(cuinstr)];	
		TRAPMUX_OUT = GetTRAPMUX(cuinstr) ? VECMUX_OUT : ImmN(8, NEXT_LATCHES.IR);
		ADDX200_OUT = GetADDX200(cuinstr) ? (0x200 + 2*TRAPMUX_OUT) : (2*TRAPMUX_OUT);
		NEXT_LATCHES.INTV = Low16bits(ADDX200_OUT);
	}
	if(GetLD_PSR(cuinstr)) {
		int psr = Low16bits(BUS);
		NEXT_LATCHES.PSR = psr;
		loadPSR_CC(psr);
	}
	if(GetLD_PROT(cuinstr)) {
		NEXT_LATCHES.PSR = setNthBit(NEXT_LATCHES.PSR, 15, 0);
	}
