...
void dcache_access(int dcache_addr, int *read_word, int write_word, int *dcache_r, 
	...
	if (!random) { /* 1 in 9 chance of read failure */
		*dcache_r = 0;
		*read_word = 0xfeed ;
	}
	else {
...
#define IR15TO11MASK 0xf800
int cs_addr_from_ir(int ir) {
	int ir15to11 = (IR15TO11MASK & ir) / (1 << 11);
	int ir5 = NthBit(5, ir);

	int cs = ir5 + (ir15to11 << 1);
	return cs;
}
...
/************************* MEM_stage() *************************/
int mem_pcmux, mem_target_pc, mem_trap_pc, mem_sr_v,
	mem_data_in, mem_dcache_out, mem_data_out, mem_r, v_mem_en, 
	mem_we0, mem_we1, mem_data_size, mem_unaligned,
	v_mem_ld_cc, v_mem_ld_reg;

int we0Bit(int isWord, int unaligned, int write) {
	return write && (isWord || !unaligned);
}

int we1Bit(int isWord, int unaligned, int write) {
	return write && (isWord || unaligned);
}
...
#define PCMUX_PCP2 0
#define PCMUX_TARGETPC 1
#define PCMUX_TRAPPC 2

#define IR_NBit(x) NthBit(11,x)
#define IR_ZBit(x) NthBit(10,x)
#define IR_PBit(x) NthBit(9,x)
#define CC_NBit(x) NthBit(2,x)
#define CC_ZBit(x) NthBit(1,x)
#define CC_PBit(x) NthBit(0,x)
void MEM_stage() {

	/* MEMORY */
	mem_target_pc = PS.MEM_ADDRESS;

	mem_data_size = Get_DATA_SIZE(PS.MEM_CS);
	mem_unaligned = Lowbit(PS.MEM_ADDRESS);
	
	mem_data_in = mem_data_size ? 
		PS.MEM_ALU_RESULT :
		(PS.MEM_ALU_RESULT << (mem_unaligned ? 8 : 0));
	mem_we0 = we0Bit(mem_data_size, mem_unaligned, Get_DCACHE_RW(PS.MEM_CS));
	mem_we1 = we1Bit(mem_data_size, mem_unaligned, Get_DCACHE_RW(PS.MEM_CS));
	v_mem_en = Get_DCACHE_EN(PS.MEM_CS) && PS.MEM_V;
	
	if(v_mem_en) {
		dcache_access(PS.MEM_ADDRESS, &mem_dcache_out, mem_data_in, &mem_r, 
			mem_we0, mem_we1);
	}
	if(!v_mem_en || Get_DCACHE_RW(PS.MEM_CS)) {
			mem_dcache_out = 0;
	}

	mem_data_out = mem_data_size ?
		mem_dcache_out :
		(mem_unaligned ? signedByteToWord(High8bits(mem_dcache_out)) :
		 				 signedByteToWord(Low8bits(mem_dcache_out)));

	mem_trap_pc = mem_data_out;

	/* STALLING */
	mem_stall = v_mem_en && !mem_r;
	...
	/* PCMUX */
	if(!PS.MEM_V) {
		mem_pcmux = PCMUX_PCP2;
	}
	else if(Get_BR_OP(PS.MEM_CS)) {
		int IR = PS.MEM_IR;
		int CC = PS.MEM_CC;
		mem_pcmux = ((IR_NBit(IR) && CC_NBit(CC)) || 
					(IR_ZBit(IR) && CC_ZBit(CC)) ||
					(IR_PBit(IR) && CC_PBit(CC))) ?
			PCMUX_TARGETPC : PCMUX_PCP2;
	}
	else if (Get_UNCOND_OP(PS.MEM_CS)) {
		mem_pcmux = PCMUX_TARGETPC;
	}
	else if (Get_TRAP_OP(PS.MEM_CS)) {
		mem_pcmux = PCMUX_TRAPPC;
	}
	else {
		mem_pcmux = PCMUX_PCP2;
	}
	...
	/* VALID */
	mem_sr_v = PS.MEM_V && !mem_stall;

	/* LATCH */
	NEW_PS.SR_ADDRESS = PS.MEM_ADDRESS;
	NEW_PS.SR_DATA = mem_data_out;
...
int perform_shift(int instr, int op) {
...
int perform_alu(int aluA, int aluB, int aluk) {	
	switch(aluk) {
		case ALUK_ADD:
			return add(aluA, aluB);
		case ALUK_AND:
			return and(aluA, aluB);
		case ALUK_XOR:
			return xor(aluA, aluB);
		case ALUK_PASSB:
			return aluB;
	}
}
...
int agex_ld_mem, agex_mem_v,
	agex_addr1, agex_addr2_preshift, agex_addr2, agex_addr_sum, agex_trapval, agex_addr_out,
	agex_shf_out, agex_aluk, agex_alu_a, agex_alu_b, agex_alu_out, agex_result,
	v_agex_ld_cc, v_agex_ld_reg, v_agex_br_stall,
	agex_stall_ahead;

void AGEX_stage() {

	/* ADDR */
	agex_addr1 = Get_ADDR1MUX(PS.AGEX_CS) ? PS.AGEX_SR1 : PS.AGEX_NPC;

	int agex_addr2mux_lookup[4] = {
		0,
		signedImmN(6, PS.AGEX_IR),
		signedImmN(9, PS.AGEX_IR),
		signedImmN(11, PS.AGEX_IR)
	};
	agex_addr2_preshift = agex_addr2mux_lookup[Get_ADDR2MUX(PS.AGEX_CS)];
	agex_addr2 = agex_addr2_preshift * (Get_LSHF1(PS.AGEX_CS) ? 2 : 1);

	agex_addr_sum = agex_addr1 + agex_addr2;
	agex_trapval = ImmN(8, PS.AGEX_IR) * 2;
	agex_addr_out = Low16bits(Get_ADDRESSMUX(PS.AGEX_CS) ? agex_addr_sum : agex_trapval);

	/* RESULT */
	agex_shf_out = perform_shift(PS.AGEX_IR, PS.AGEX_SR1);

	agex_alu_a = PS.AGEX_SR1;
	agex_alu_b = Get_SR2MUX(PS.AGEX_CS) ? signedImmN(5, PS.AGEX_IR) : PS.AGEX_SR2;
	agex_alu_out = perform_alu(agex_alu_a, agex_alu_b, Get_ALUK(PS.AGEX_CS));

	agex_result = Low16bits(Get_ALU_RESULTMUX(PS.AGEX_CS) ? agex_alu_out : agex_shf_out);
	...
	/* LD/VALID */
	agex_mem_v = PS.AGEX_V;
	
	agex_stall_ahead = mem_stall;
	agex_ld_mem = !agex_stall_ahead;


	/* LATCH */
	if (agex_ld_mem) {
		NEW_PS.MEM_ADDRESS = agex_addr_out;
		...
		NEW_PS.MEM_ALU_RESULT = agex_result;
		NEW_PS.MEM_IR = PS.AGEX_IR;
		NEW_PS.MEM_DRID = PS.AGEX_DRID;
		NEW_PS.MEM_V = agex_mem_v;
	}
}
...
int is_data_hazard(int de_sr,int de_sr_needed, int future_ld_reg, int future_drid) {
	return de_sr_needed && future_ld_reg && (de_sr == future_drid);
}

int de_sr1, de_sr1_out, de_sr2, de_sr2_out, de_stall_ahead, de_stall;
int de_cc_out, de_br_stall, de_drid, de_agex_v, de_ld_agex, de_cs_addr;
int * de_cs;
int de_sr1_needed, de_sr2_needed, de_br_op;

void DE_stage() {

	/* CS */
	de_cs_addr = cs_addr_from_ir(PS.DE_IR);
	de_cs = CONTROL_STORE[de_cs_addr];

	/* SRs */
	de_sr1 = RegMid(PS.DE_IR);
	de_sr1_out = REGS[de_sr1];

	int de_sr2_mux_lookup[2] = {RegLow(PS.DE_IR), RegHigh(PS.DE_IR)};
	int de_sr2_id_mux = NthBit(13,PS.DE_IR);
	de_sr2 = de_sr2_mux_lookup[de_sr2_id_mux];
	de_sr2_out = REGS[de_sr2];
	
	if(v_sr_ld_reg) {
		REGS[sr_drid] = Low16bits(sr_reg_data);
	}

	/* CCs */
	de_cc_out = (N << 2) + (Z << 1) + P; /* before because no bypassing */
	if(v_sr_ld_cc) {
		N = Lowbit(sr_n);
		Z = Lowbit(sr_z);
		P = Lowbit(sr_p);
	...
	/* DRID */
	int de_drmux_lookup[2] = {RegHigh(PS.DE_IR),7};
	int de_drmux = Get_DRMUX(de_cs);
	de_drid = de_drmux_lookup[de_drmux];

	/* STALLING */
	v_de_br_stall = PS.DE_V && Get_DE_BR_STALL(de_cs);

	de_sr1_needed = Get_SR1_NEEDED(de_cs);
	de_sr2_needed = Get_SR2_NEEDED(de_cs);
	de_br_op = Get_DE_BR_OP(de_cs);

	int de_control_hazard = de_br_op && (v_agex_ld_cc || v_mem_ld_cc || v_sr_ld_cc); 
	dep_stall = PS.DE_V && 
		(de_control_hazard ||
		 is_data_hazard(de_sr1, de_sr1_needed, v_agex_ld_reg, PS.AGEX_DRID) ||
		 is_data_hazard(de_sr1, de_sr1_needed, v_mem_ld_reg, PS.MEM_DRID) ||
		 is_data_hazard(de_sr1, de_sr1_needed, v_sr_ld_reg, PS.SR_DRID) ||
		 is_data_hazard(de_sr2, de_sr2_needed, v_agex_ld_reg, PS.AGEX_DRID) ||
		 is_data_hazard(de_sr2, de_sr2_needed, v_mem_ld_reg, PS.MEM_DRID) ||
		 is_data_hazard(de_sr2, de_sr2_needed, v_sr_ld_reg, PS.SR_DRID));

	/* LD/VALID */
	de_stall = dep_stall;
	de_stall_ahead = mem_stall;

	de_ld_agex = !de_stall_ahead;
	de_agex_v = PS.DE_V && !de_stall_ahead && !de_stall;
	...
	if (de_ld_agex) {
		...
		for (ii = COPY_AGEX_CS_START; ii< NUM_CONTROL_STORE_BITS; ii++) {
			NEW_PS.AGEX_CS[jj++] = CONTROL_STORE[de_cs_addr][ii];
		}

		NEW_PS.AGEX_IR = PS.DE_IR;
		NEW_PS.AGEX_SR1 = de_sr1_out;
		NEW_PS.AGEX_SR2 = de_sr2_out;
		NEW_PS.AGEX_CC = de_cc_out;
		NEW_PS.AGEX_DRID = de_drid;
		NEW_PS.AGEX_V = de_agex_v;
	}
...
int fe_pc_plus_two, fe_ld_de, fe_ld_pc, fe_pc_in, fe_de_v, fe_icache_out;
int fe_stall_ahead, fe_br_stall_ahead, fe_any_stall_ahead;

void FETCH_stage() {

	/* pc_in */
	fe_pc_plus_two = PC + 2;
	int pc_in_vec[3] = {fe_pc_plus_two, mem_target_pc, mem_trap_pc};
	fe_pc_in = pc_in_vec[mem_pcmux];

	/* icache_out, icache_r */
	icache_access(PC, &fe_icache_out, &icache_r);

	/* LD_PC */
	fe_stall_ahead = dep_stall || mem_stall;
	fe_br_stall_ahead = v_de_br_stall || v_agex_br_stall || v_mem_br_stall;
	fe_any_stall_ahead = fe_br_stall_ahead || fe_stall_ahead;

	fe_ld_pc = (v_mem_br_stall && mem_pcmux != PCMUX_PCP2) ||
		(!fe_any_stall_ahead && icache_r); 

	/* de_v */
	fe_de_v = icache_r && !fe_br_stall_ahead;
	
	/* LD_DE */
	fe_ld_de = !fe_stall_ahead;

	/* Latch everything */
	if(fe_ld_de) {
		NEW_PS.DE_NPC = fe_pc_plus_two;
		NEW_PS.DE_IR = fe_icache_out;
		NEW_PS.DE_V = fe_de_v;
	}
	if(fe_ld_pc) {
		PC = fe_pc_in;
